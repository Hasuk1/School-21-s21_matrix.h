...
1. Была разработана библиотека на языке C11 с использованием компилятора gcc.
2. Исходный код библиотеки находится в папке "src" в ветке "develop".
3. При написании кода были избегены устаревшие и выведенные из употребления конструкции языка и библиотечные функции, ориентировались на стандарт POSIX.1-2017 и придерживались Google Style.
4. Решение было оформлено в виде статической библиотеки соответствующего заголовочного файла "s21_matrix.h".
5. Библиотека была разработана в соответствии с принципами структурного программирования.
6. Была подготовлена полная покрытие unit-тестами функций библиотеки с использованием библиотеки Check. Unit-тесты покрывали не менее 80% каждой функции.
7. Создан Makefile для сборки библиотеки и тестов с целями "all", "clean", "test", "s21_matrix.a", "gcov_report".
8. В цели "gcov_report" формировался отчет gcov в виде HTML страницы. Для этого unit-тесты запускались с флагами gcov.
9. Матрица была реализована в виде структуры согласно указанному описанию.
10 .Была обеспечена точность дробной части матрицы до максимум 6 знаков после запятой.
